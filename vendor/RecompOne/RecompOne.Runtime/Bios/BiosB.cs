...
{
    static readonly bool TraceInput =
        Environment.GetEnvironmentVariable("RECOMPONE_TRACE_INPUT") == "1";
    ...
    enum CardEventLevel { LowerIo, HigherDevice }
    static readonly Queue<(uint Port, CardEventLevel Level)> _cardEvents = new();

    public static uint IntrEnvInInterruptAddr = 0u;

    static uint _padBuf;
    static uint _padBuf1;
    static uint _padBuf2;
    static uint _padBuf1Size;
    ...
    public static void QueueCardInfo(uint port) =>
        _cardEvents.Enqueue((port, CardEventLevel.HigherDevice));

    static void QueueCardIo(uint port) =>
        _cardEvents.Enqueue((port, CardEventLevel.LowerIo));

    public static void TickCards(CpuContext c, IMemory m)
    {
        if (_cardEvents.Count == 0) return;
        var (port, level) = _cardEvents.Dequeue();
        var card = (port & 0x10u) != 0 ? Runtime.CardB : Runtime.CardA;
        uint spec = card.Enabled ? 0x0004u : 0x0100u;
        Log.Bios(
            $"card completion port=0x{port:X8} level={level} " +
            $"enabled={card.Enabled} spec=0x{spec:X4}");

        uint eventClass = level == CardEventLevel.HigherDevice
            ? 0xF4000001u
            : 0xF0000011u;
        DeliverEventIntr(c, m, eventClass, spec);
    }
    ...
    static void PadRead(IMemory m)
    {
        ...
        if (_padDirectRunning)
        {
            WriteDirectPad(m, _padBuf1, _padBuf1Size, s,
                Hardware.Controller.RightX, Hardware.Controller.RightY,
                Hardware.Controller.LeftX, Hardware.Controller.LeftY);
            WriteDirectPad(m, _padBuf2, _padBuf2Size, s2,
                0x80, 0x80, 0x80, 0x80);
        }

        if (TraceInput && (s != 0xFFFF || s2 != 0xFFFF))
            Console.Error.WriteLine(
                $"[Input] BIOS pad buffers=0x{_padBuf:X8}/" +
                $"0x{_padBuf1:X8}/0x{_padBuf2:X8} running={_padDirectRunning} " +
                $"p1=0x{s:X4} p2=0x{s2:X4}");
    }

    static void WriteDirectPad(
        IMemory m, uint buffer, uint size, ushort state,
        byte rightX, byte rightY, byte leftX, byte leftY)
    {
        if (buffer == 0 || size < 4) return;

        // InitPAD2 buffers contain the controller's serial response without
        // the leading 0xFF sync byte: status, controller ID, then payload.
        m.WriteU8(buffer, 0x00);
        m.WriteU8(buffer + 1, size >= 8 ? (byte)0x73 : (byte)0x41);
        m.WriteU8(buffer + 2, (byte)state);
        m.WriteU8(buffer + 3, (byte)(state >> 8));
        if (size >= 8)
        {
            m.WriteU8(buffer + 4, rightX);
            m.WriteU8(buffer + 5, rightY);
            m.WriteU8(buffer + 6, leftX);
            m.WriteU8(buffer + 7, leftY);
        }
    }
    ...
    public static void Dispatch(CpuContext c, IMemory m, uint fn)
    {
        ...
        switch (fn)
        {
            ...
            case 0x12:
                ...
                if (TraceInput)
                    Console.Error.WriteLine(
                        $"[Input] BIOS InitPAD2 p1=0x{_padBuf1:X8}/0x{_padBuf1Size:X} " +
                        $"p2=0x{_padBuf2:X8}/0x{_padBuf2Size:X}");
                break;
            case 0x13:
                _padDirectRunning = true;
                PadRead(m);
                c.V0 = 1u;
                if (TraceInput)
                    Console.Error.WriteLine("[Input] BIOS StartPAD2");
                break;
            case 0x14:
                _padDirectRunning = false;
                c.V0 = 1u;
                if (TraceInput)
                    Console.Error.WriteLine("[Input] BIOS StopPAD2");
                break;
            case 0x15:
                _padBuf = c.A1;
                if (TraceInput)
                    Console.Error.WriteLine(
                        $"[Input] BIOS PAD_init mode=0x{c.A0:X8} buffer=0x{_padBuf:X8}");
                break;
            ...
            case 0x4D: QueueCardInfo(c.A0); c.V0 = 1u; break;
            case 0x4E: CardWrite(c, m); break;
            case 0x4F: CardRead(c, m); break;
            case 0x50: c.V0 = 1u; break;
    ...
    {
        Log.Bios(
            $"OpenEvent class=0x{@class:X8} spec=0x{spec:X8} " +
            $"mode=0x{mode:X8} func=0x{func:X8}");
        for (int i = 0; i < MaxEvents; i++)
        {
