...
public sealed class CueFs : IDisposable
{
    ...
    public bool IsMusicLba(int lba) =>
        TryDescribeLba(lba, out string path, out _) &&
        Path.GetFileName(path).Equals(
            "MUSIC.DAT", StringComparison.OrdinalIgnoreCase);
        ...
        long? sourceLength = null)
    {
        ...
        bool exactRawSectorDump =
            hostSize > 0 &&
            hostSize == sectors * 2336L;
        LooseStorage storage = (IsStreamPath(discPath) || exactRawSectorDump) &&
            hostSize % 2336 == 0
    ...
    private byte[] ReadLooseSector(LooseEntry entry, int sectorIndex, int size)
    {
        ...
        if (entry.Storage == LooseStorage.Raw2336)
        {
            ...
            return size switch
            {
                ...
                2336 => raw,
                2340 => AddSectorHeader(raw, lba),
                2352 => AddSyncAndSectorHeader(raw, lba),
        ...
        return size switch
        {
            ...
            2336 => AddSubheaderAndSuffix(cooked),
            2340 => AddSectorHeader(AddSubheaderAndSuffix(cooked), lba),
            2352 => AddSyncAndSectorHeader(AddSubheaderAndSuffix(cooked), lba),
            _ => throw new NotSupportedException(
                $"Loose cooked-sector size {size} is not supported"),
        };
    }

    private static byte[] AddSectorHeader(byte[] raw2336, int lba)
    {
        var result = new byte[2340];
        WriteSectorHeader(result, 0, lba);
        raw2336.CopyTo(result, 4);
        return result;
    }

    private static byte[] AddSyncAndSectorHeader(byte[] raw2336, int lba)
    {
        var result = new byte[2352];
        result[0] = 0x00;
        Array.Fill(result, (byte)0xFF, 1, 10);
        result[11] = 0x00;
        WriteSectorHeader(result, 12, lba);
        raw2336.CopyTo(result, 16);
        return result;
    }

    private static void WriteSectorHeader(byte[] destination, int offset, int lba)
    {
        int absolute = lba + 150;
        destination[offset] = ToBcd(absolute / (60 * 75));
        destination[offset + 1] = ToBcd((absolute / 75) % 60);
        destination[offset + 2] = ToBcd(absolute % 75);
        destination[offset + 3] = 0x02;
    }

    private static byte ToBcd(int value) =>
        checked((byte)(((value / 10) << 4) | (value % 10)));

    private static byte[] AddSubheaderAndSuffix(byte[] cooked)
    {
        var result = new byte[2336];
        // A normal Mode 2 Form 1 data sector. Raw stream files retain their
        // original per-sector XA subheaders through the Raw2336 path above.
        result[2] = result[6] = 0x08;
        cooked.CopyTo(result, 8);
        return result;
    }
