...
public sealed class PSMemory : IMemory
{
    ...
    void NotifyProjectedRamWords(uint phys, int size)
    {
        if (!TryGetProjectedWordAddress(phys, out uint firstAddress))
            return;

        uint first = firstAddress & ~3u;
        NotifyProjectedRamWord(first);
        if (TryGetProjectedWordAddress(
                phys + (uint)size - 1u, out uint lastAddress))
        {
            uint last = lastAddress & ~3u;
            if (last != first)
                NotifyProjectedRamWord(last);
        }
    }

    bool TryGetProjectedWordAddress(uint phys, out uint projectedAddress)
    {
        if (phys < MemoryMap.RamWindow)
        {
    ...
    void NotifyProjectedRamWord(uint projectedAddress)
    {
        uint value;
    ...
    public uint ReadU32(uint address)
    {
        ...
        if ((phys & 3u) == 0 &&
            TryGetProjectedWordAddress(phys, out uint projectedAddress))
            Gte.NotifyRamRead(projectedAddress, value);
        return value;
    }
