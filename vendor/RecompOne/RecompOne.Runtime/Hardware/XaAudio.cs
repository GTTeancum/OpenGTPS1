...
public static class XaAudio
{
    ...
    static readonly int PrimeFrames =
        int.TryParse(
            Environment.GetEnvironmentVariable("RECOMPONE_XA_PRIME_FRAMES"),
            out int configuredPrimeFrames)
            ? Math.Clamp(configuredPrimeFrames, 1024, 16384)
            : 4096;
    ...
    static long _healthUnderflowFrames;
    static int _healthRefills;
    static int _healthMaximumGap;
        ...
        lock (_gate)
        {
            ...
            if (_trace && (decoded == 1 || decoded % 19 == 0))
            {
                Console.Error.WriteLine(
                    ...
                    $"rate={rate} frames={n} buffered={_count} prime={PrimeFrames} " +
                    $"peak={peak}");
            }
        }
    }
    ...
    public static bool Next(out short left, out short right)
    {
        lock (_gate)
        {
            ...
            while (_pos >= 1.0)
            {
                ...
                {
                    _underrun++;
                    _healthUnderflowFrames++;
                    _healthMaximumGap = Math.Max(_healthMaximumGap, _underrun);
    ...
    public static (long underflowFrames, int refills, int maximumGap)
        GetHealthAndReset()
    {
        lock (_gate)
        {
