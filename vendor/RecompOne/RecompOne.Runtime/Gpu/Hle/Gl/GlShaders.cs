...
internal static class GlShaders
{
    ...
    public const string PrimVs = """
        ...
        // PS1 Gouraud colour is interpolated linearly in screen space. Only
        // texture coordinates should use the synthetic clip W that enables
        // perspective-correct texture projection.
    ...
    public const string PrimFs = """
        ...
        void main() {
            ...
            if (texMode == 4) {
                FragColor = vec4(
                    quant5(ivec3(vColor.rgb * 255.0 + 0.5)),
                    uSetMask);
                BlendColor = uBlend;
                return;
            }
