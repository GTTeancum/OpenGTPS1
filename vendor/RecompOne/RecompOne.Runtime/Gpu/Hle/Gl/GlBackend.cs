...
public sealed class GlBackend : IGpuBackend
{
    ...
    long _rtStamp;
    long _frame;
    long _frameTriangles;
    long _frameTexturedTriangles;
    long _frameDepthTriangles;
    long _framePerspectiveTriangles;
    long _frameSeamTriangles;
    long _frameDitherTriangles;
    long _windowTriangles;
    long _windowTexturedTriangles;
    long _windowDepthTriangles;
    long _windowPerspectiveTriangles;
    long _windowSeamTriangles;
    long _windowDitherTriangles;
    static readonly bool TracePrimitives =
        Environment.GetEnvironmentVariable("RECOMPONE_TRACE_GPU_PRIMITIVES") == "1";
    ...
    public void DrawTri(in HleVertex a, in HleVertex b, in HleVertex c, in PrimFlags f)
    {
        _frameTriangles++;
        if (f.Textured)
            _frameTexturedTriangles++;
        bool dith = DitherOf(f);
        if (dith)
            _frameDitherTriangles++;
        bool hasDepth =
            f.Textured &&
            HasCoherentPerspectiveDepth(a, b, c);
        if (hasDepth)
            _frameDepthTriangles++;
        bool perspectiveCorrect =
            ConfigManager.View.PerspectiveCorrectTextures && hasDepth;
        if (perspectiveCorrect)
            _framePerspectiveTriangles++;
        bool seamEligible =
            !f.SemiTrans &&
            !_env.SetMask &&
            !_env.CheckMask &&
        ...
        var vc = V(c, f, dith, perspectiveCorrect);
        if (seamPadding)
        {
            // Draw padding first, then the unmodified guest triangle. This
            // confines altered interpolation to the conservative border; the
            // original triangle restores every interior pixel exactly.
            var pa = va;
            var pb = vb;
            var pc = vc;
            ExpandProjectedTriangle(ref pa, ref pb, ref pc);
            _verts[_count++] = pa;
            _verts[_count++] = pb;
            _verts[_count++] = pc;
            _frameSeamTriangles++;
    ...
    static bool HasCoherentPerspectiveDepth(
        in HleVertex a,
        in HleVertex b,
        in HleVertex c)
    {
        if (!a.HasGteZ || !b.HasGteZ || !c.HasGteZ)
            return false;
        float minimum = MathF.Min(a.Z, MathF.Min(b.Z, c.Z));
        float maximum = MathF.Max(a.Z, MathF.Max(b.Z, c.Z));
        return minimum > 0f &&
            maximum / minimum <= Gte.MaximumPerspectiveDepthRatio;
    }

    static void ExpandProjectedTriangle(
        ref GlVertex a, ref GlVertex b, ref GlVertex c)
    {
        // Adjacent PS1 meshes are projected and rounded independently. At a
        // high internal resolution that native subpixel disagreement becomes
        // a visible hairline. Offset every edge by a fixed native-pixel
        // distance instead of moving vertices radially from the centroid.
        // Centroid expansion barely moves a long edge at its acute endpoint,
        // which left the intermittent Red Rock crack visible at replay
        // 0:24.000. Mitered edge normals give every ordinary edge the same
        // conservative coverage. Clamp pathological skinny-triangle miters
        // so padding cannot reach unrelated geometry.
        const float overlap = 0.75f;
        const float maximumMiter = 2.0f;
        float area =
            (b.X - a.X) * (c.Y - a.Y) -
            (b.Y - a.Y) * (c.X - a.X);
        ...
        if (!TryGetOutwardNormal(
                in a, in b, winding, out float abX, out float abY) ||
            !TryGetOutwardNormal(
                in b, in c, winding, out float bcX, out float bcY) ||
            !TryGetOutwardNormal(
                in c, in a, winding, out float caX, out float caY))
            return;

        ExpandProjectedVertex(
            ref a, caX, caY, abX, abY, overlap, maximumMiter);
        ExpandProjectedVertex(
            ref b, abX, abY, bcX, bcY, overlap, maximumMiter);
        ExpandProjectedVertex(
            ref c, bcX, bcY, caX, caY, overlap, maximumMiter);
    }

    static bool TryGetOutwardNormal(
        in GlVertex from,
        in GlVertex to,
        float winding,
        out float normalX,
        out float normalY)
    {
        ...
        normalX = winding * dy / length;
        normalY = -winding * dx / length;
        return true;
    }

    static void ExpandProjectedVertex(
        ref GlVertex vertex,
        float firstNormalX,
        float firstNormalY,
        float secondNormalX,
        ...
        float maximumMiter)
    {
        float denominator =
            1f +
            firstNormalX * secondNormalX +
            firstNormalY * secondNormalY;
        if (denominator < 0.0001f)
            return;

        float scale = overlap / denominator;
        float offsetX = (firstNormalX + secondNormalX) * scale;
        float offsetY = (firstNormalY + secondNormalY) * scale;
        float length = MathF.Sqrt(offsetX * offsetX + offsetY * offsetY);
        if (length > maximumMiter)
        {
            float clamp = maximumMiter / length;
            offsetX *= clamp;
            offsetY *= clamp;
        }
        vertex.X += offsetX;
    ...
    public unsafe (uint tex, int w, int h, float aspect) PresentDisplay(int dispX, int dispY, int w, int h, bool rgb24 = false, int outW = 0, int outH = 0)
    {
        ...
        _frame++;
        Flush();
        _windowTriangles += _frameTriangles;
        _windowTexturedTriangles += _frameTexturedTriangles;
        _windowDepthTriangles += _frameDepthTriangles;
        _windowPerspectiveTriangles += _framePerspectiveTriangles;
        _windowSeamTriangles += _frameSeamTriangles;
        _windowDitherTriangles += _frameDitherTriangles;
        if (TracePrimitives &&
            (_frame <= 20 || (_frame % 120) == 0))
        {
            Console.Error.WriteLine(
                $"[GPU-Primitives] frame={_frame} triangles={_frameTriangles} " +
                $"textured={_frameTexturedTriangles} depth={_frameDepthTriangles} " +
                $"perspective={_framePerspectiveTriangles} " +
                $"seamFixed={_frameSeamTriangles} " +
                $"dithered={_frameDitherTriangles} " +
                $"windowTriangles={_windowTriangles} " +
                $"windowTextured={_windowTexturedTriangles} " +
                $"windowDepth={_windowDepthTriangles} " +
                $"windowPerspective={_windowPerspectiveTriangles} " +
                $"windowSeamFixed={_windowSeamTriangles} " +
                $"windowDithered={_windowDitherTriangles}");
            _windowTriangles = 0;
            _windowTexturedTriangles = 0;
            _windowDepthTriangles = 0;
            _windowPerspectiveTriangles = 0;
            _windowSeamTriangles = 0;
            _windowDitherTriangles = 0;
        }
        _frameTriangles = 0;
        _frameTexturedTriangles = 0;
        _frameDepthTriangles = 0;
        _framePerspectiveTriangles = 0;
        _frameSeamTriangles = 0;
        _frameDitherTriangles = 0;
