...
public sealed partial class Gpu
{
    struct Vert
    {
        ...
        public int DepthAge;
        public GteDepthProvenance DepthProvenance;
    }
    ...
    void DrawPolygon()
    {
        ...
        for (int i = 0; i < n; i++)
        {
                    ...
                    out origins[i]))
            {
                v[i].Z = z;
                v[i].DepthAge = age;
                v[i].DepthProvenance = provenance;
                v[i].HasGteZ =
                    provenance is
                        GteDepthProvenance.DirectStore or
        ...
        if (ProjectionTrace.Enabled && tex)
        {
            Span<ProjectionTraceVertex> traceVertices =
                stackalloc ProjectionTraceVertex[n];
            for (int i = 0; i < n; i++)
            {
                traceVertices[i] = new ProjectionTraceVertex(
                    v[i].X,
                    v[i].Y,
                    v[i].U,
                    v[i].V,
