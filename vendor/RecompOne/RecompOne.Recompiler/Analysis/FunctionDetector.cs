...
public static class FunctionDetector
{
    ...
    public static MipsFunction? DetectExact(MipsInstruction[] all, uint address, int size, string? nameHint, string overlayName)
    {
        if (all.Length == 0 || size <= 0 || (size & 3) != 0) return null;

        int startIdx = InstrIndex(all, address);
        if (startIdx < 0 || startIdx >= all.Length) return null;

        ulong endAddress = (ulong)address + (uint)size;
        ulong codeEnd = (ulong)all[^1].Vram + 4;
        if (endAddress > codeEnd) return null;

        int endIdx = InstrIndex(all, (uint)endAddress);
        if (endIdx <= startIdx || endIdx > all.Length) return null;
        if (SliceHasUnknownInstruction(all, startIdx, endIdx)) return null;

        string name = nameHint ?? $"func_{address:X8}";
