...
public static class EntryWriter
{
    public static void Write(PsxExe exe, string bootExe, uint systemStack, string className, string windowTitle, string? mainCall, List<string> overlays, string outDir)
    {
        ...
        entry.AppendLine("        if (loosePath != null)");
        entry.AppendLine("            RecompOne.Runtime.OggMusic.Initialize(loosePath);");
        ...
        uint initialSp = exe.InitialSP != 0 ? exe.InitialSP : systemStack;
        entry.AppendLine($"        c.SP = 0x{initialSp:X8}u;");
