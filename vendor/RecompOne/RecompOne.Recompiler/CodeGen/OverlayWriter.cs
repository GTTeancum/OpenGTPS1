...
public static class OverlayWriter
{
    record OverlayResult(string Name, List<MipsFunction> Functions, int LbaStart,
        uint Base, uint Size, uint ImageSize, bool Relocatable,
    ...
    public static void Write(RecompOneConfig config, CueFs fs, string outDir)
    {
...

        {
            ...
            overlayResults.Add(new OverlayResult(
                "main", funcs, -1, 0, 0, 0, false, mainInstrs));
        }
        ...
        foreach (var overlayConfig in config.Overlays)
        {
            ...
            overlayResults.Add(new OverlayResult(overlayConfig.Name, funcs, overlayLba,
                ovlBase, (uint)discBin.Length, imageSize,
                overlayConfig.V8Relocate, instrs));
        }
    ...
    static void AddConfigFunctions(List<MipsFunction> funcs, Config.FunctionEntry[] entries, MipsInstruction[] instrs, IEnumerable<Symbols.FunctionEntry> noTypeSymbols, string overlayName)
    {
        ...
        int exactCount = 0;
        foreach (var entry in entries.Where(e => e.Size is > 0))
        {
            uint address = Convert.ToUInt32(entry.Address, 16);
            var exact = FunctionDetector.DetectExact(instrs, address, entry.Size!.Value, entry.Name, overlayName)
                ?? throw new InvalidDataException(
                    $"invalid exact function {entry.Address} size 0x{entry.Size.Value:X} in {overlayName}");

            int existingIndex = funcs.FindIndex(f => f.Start == address);
            if (existingIndex >= 0)
                funcs[existingIndex] = exact;
            else
                funcs.Add(exact);
            exactCount++;
        }
        ...
        var missing = entries
            .Where(f => f.Size is null or <= 0)
            .Select(f => (Addr: Convert.ToUInt32(f.Address, 16), f.Name))
            .Where(e => have.Add(e.Addr))
            .ToList();

        var extras = missing.Count == 0
            ? []
            : FunctionDetector.DetectFromAddresses(instrs, missing.Select(e => (e.Addr, e.Name)), funcs, overlayName);
        if (extras.Count > 0) funcs.AddRange(extras);
        var callees = FunctionDetector.DiscoverCalls(instrs, funcs, noTypeSymbols, overlayName);
        funcs.AddRange(callees);
        Console.WriteLine($"[Recompiler] added {extras.Count} config function(s), fixed {exactCount} exact function(s) (+{callees.Count} callees) to {overlayName}");
    }
        ...
        bool relocatable, MipsInstruction[] instrs, string outDir)
    {
        ...
        foreach (var func in funcs.OrderBy(f => f.Start))
        {
            ...
            var ctx = new FunctionContext
            {
                ...
                RelocatableOverlay = relocatable
            };
        ...
        sb.AppendLine($"    public bool Relocatable => {relocatable.ToString().ToLowerInvariant()};");
    ...
    static (byte[]? data, int lba) ResolveOverlay(CueFs fs, Config.OverlayConfig cfg)
    {
        try
        {
            if (cfg.Lba >= 0)
            {
                ...
                return (TransformOverlay(fs.ReadSectors(cfg.Lba, sz), cfg), cfg.Lba);
            }
            if (cfg.File != null)
            {
                ...
                return (TransformOverlay(full.AsSpan(start, length).ToArray(), cfg), absLba);
            }
    ...
    static byte[] TransformOverlay(byte[] data, Config.OverlayConfig cfg)
    {
        data = Decrypt(data, cfg.Decrypt);
        if (!cfg.Gzip) return data;
